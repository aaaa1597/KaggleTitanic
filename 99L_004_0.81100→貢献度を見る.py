...
# 1. パイプラインから学習済みのモデル（RandomForestClassifier）を取り出す
#    make_pipelineの場合、小文字のクラス名 'randomforestclassifier' で取り出せます
rf_model = pipeline.named_steps['randomforestclassifier']

# 2. SelectKBestで選択された（残った）特徴量名だけにフィルタリングする
#    maskは上で作成した select.get_support() をそのまま使います
selected_features = [col for col, is_selected in zip(features, mask) if is_selected]

# 3. モデルから特徴量の重要度（貢献度）を取得
importances = rf_model.feature_importances_

# 4. 特徴量名と重要度をデータフレームにまとめる
df_importance = pd.DataFrame({
    'Feature': selected_features,  # 20個に絞られた特徴量名
    'Importance': importances      # 20個の重要度
...
# pipelineから取り出した rf_model を使用する
estimator = rf_model.estimators_[0]

# 視覚化（特徴量名は selected_features を指定）
plt.figure(figsize=(20, 10))
plot_tree(estimator, 
          feature_names=selected_features,  # 20個に絞られた特徴量名
